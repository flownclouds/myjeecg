...
public class JeecgReadTable {
  ...
  {
    dbType=CodeResourceUtil.DATABASE_TYPE;
    dbDriver=CodeResourceUtil.DIVER_NAME;
    dbUrl=CodeResourceUtil.URL;
    dbUser=CodeResourceUtil.USERNAME;
    dbPass=CodeResourceUtil.PASSWORD;
    dbName=CodeResourceUtil.DATABASE_NAME;
  }
  public JeecgReadTable(String dbJson)
  {
    Gson gs=new Gson();
    @SuppressWarnings("unchecked")
    HashMap< String,String > hm= gs.fromJson(dbJson, HashMap.class);
    dbType=hm.get("type");
    dbDriver=hm.get("driver");
    dbUrl=hm.get("url");
    dbUser=hm.get("user");
    dbPass=hm.get("pass");
    dbName=hm.get("dbname");
  }
  ...
  public List<String> readAllTableNames() throws SQLException {
    ...
    try {
      ...
      if(dbType.equals("mysql")) {
        this.sql = MessageFormat.format("select distinct table_name from information_schema.columns where table_schema = {0}", new Object[]{TableConvert.getV(dbName)});
      }
  ...
  public List<Columnt> readTableColumn(String tableName) throws Exception {
    ...
    try {
      ...
      this.stmt = this.conn.createStatement(1005, 1007);
      if(dbType.equals("mysql")) {
        this.sql = MessageFormat.format("select column_name,data_type,column_comment,numeric_precision,numeric_scale,character_maximum_length,is_nullable nullable from information_schema.columns where table_name = {0} and table_schema = {1}", new Object[]{TableConvert.getV(tableName.toUpperCase()), TableConvert.getV(dbName)});
      }
  ...
  public boolean checkTableExist(String tableName) {
    try {
      ...
      this.stmt = this.conn.createStatement(1005, 1007);
      if(dbType.equals("mysql")) {
        this.sql = "select column_name,data_type,column_comment,0,0 from information_schema.columns where table_name = \'" + tableName.toUpperCase() + "\'" + " and table_schema = \'" + dbName + "\'";
      }
