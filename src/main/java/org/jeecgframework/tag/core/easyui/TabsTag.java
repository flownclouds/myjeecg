...
public class TabsTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		if (iframe) {
			...
			sb.append(" var arrttitle=[]; var iscache=false;");
...
//			sb2.append("function initTabs(){");
//			sb2.append(" var p = $(this).tabs(\'getTab\', title);");
//			sb2.append(" $.each($('#\" + id + \"').tabs(),function(i,item){var title= $('a:eq(0) span', $(item)).text();alert(title);})");
//			if (tabList.size() > 0) {
//				for (Tab tab : tabList) {
//					if(!tab.isCache()) {
//						sb2.append("if (title == \'" + tab.getTitle() + "\') {");
//						sb2.append("p.find(\'iframe\').attr(\'src\',");
//						sb2.append("\'" + tab.getHref() + "\');}");
//					}
//				}
//			}
			...
			if (tabList.size() > 0) {
				for (Tab tab : tabList) {
					...
					sb.append(" if(arrttitle.join(',').indexOf('"+tab.getTitle()+"')<0 ){" );
						sb.append("p.find(\'iframe\').attr(\'src\',");
						sb.append("\'" + tab.getHref() + "\'); if(iscache){ arrttitle.push(\'"+tab.getTitle()+"\');} }");
					sb.append("} ");
